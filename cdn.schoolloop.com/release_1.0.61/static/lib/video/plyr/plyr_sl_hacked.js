...
(function(root, factory) {
    ...
    function _toggleListener(element, events, callback, toggle, useCapture) {
        ...
        // sl.log("_toggleListener",element, events, callback, toggle)
    ...
    function Plyr(media, config) {
        ...
        function _findElements() {
            try {
                ...
                plyr.buttons.volume = _getElement(config.selectors.buttons.volume);//sl added for accessibility
        ...
        function _setupStorage() {
            ...
            if (!value) {
                ...
                // Update the key to be JSON
                  ...
                  //remmoved storage of volume as it overrides mute option Feb 15th 2018
        ...
        function _setVolume(volume) {
            sl.log("_setVolume", volume)
            var max = config.volumeMax,
                min = config.volumeMin;

            // Load volume from storage if no value specified    //Commented out to fix bug where mute was getting overwritten
        ...
        function _toggleControls(toggle) {
            sl.log("_toggleControls tabFocus",_getElement('.' + config.classes.tabFocus));
            sl.log("_toggleControls focused",document.querySelector(':focus'));
            ...
            sl.log("delay", delay, "show", show)
            if (document.querySelector(':focus') == _getElement('.' + config.classes.tabFocus)){
                show = true;
                delay = 5000;
            }
        ...
        function onBodyClick() {
            if (_getElement('.' + config.classes.tabFocus) != plyr.buttons["mute"]) {
        ...
        function _controlListeners() {
            ...
            function togglePlay() {
                ...
                var hadTabFocus = _hasClass(plyr.buttons["play"][1], config.classes.tabFocus) || _hasClass(plyr.buttons["pause"], config.classes.tabFocus);
                _checkPlaying(); // sl - to update pause play/button
                ...
                if (target) {
                    ...
                    var callback= function (hadTabFocus){
                        if (hadTabFocus) {
                    ...
                    setTimeout(callback, 100, hadTabFocus);
                }
            }
            ...
            function handleKey(event) {
                ...
                if (pressed) {
                    ...
                    switch (code) {
                        ...
                        case 38:
                        case 39:
                            if (document.activeElement == plyr.volume.input) {          //sl added for accessibility
                                _increaseVolume();
                            }
                            if (document.activeElement.parentNode == plyr.progress.container) {
                                _forward();
                            }
                            break;
                        // Arrow down
            ...
            _on(window, 'keyup', function(event) {
                var code = getKeyCode(event),
                    focused = getFocusElement();
                sl.log("keyup",code,"focused",focused);
                if (code === 9) {
